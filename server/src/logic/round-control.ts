...
/**
 * Emite todos os eventos de socket necessários após fim de turno.
 * @deprecated Esta função não está sendo usada. Os handlers emitem eventos diretamente.
 */
export function emitTurnEndEvents(
  io: Server,
  lobbyId: string,
  battleId: string,
  output: EndUnitTurnOutput,
  unitId: string,
  unit: BattleUnit,
  allUnits: BattleUnit[]
): void {
  // Evento de turno da unidade finalizado
  io.to(lobbyId).emit("battle:unit_turn_ended", {
    battleId,
    unitId,
    ...
    damageFromConditions: output.turnEndResult.damageFromConditions,
    conditionsRemoved: output.turnEndResult.conditionsRemoved,
  });

  // Se unidade morreu por condições
  if (output.turnEndResult.unitDefeated) {
    io.to(lobbyId).emit("battle:unit_defeated", {
      battleId,
      unitId,
  ...
  // Evento de próximo jogador
  io.to(lobbyId).emit("battle:next_player", {
    battleId,
    currentPlayerId: output.turnTransition.nextPlayerId,
    index: output.turnTransition.nextTurnIndex,
    round: output.turnTransition.newRound,
  });

  // Evento de nova rodada se avançou
  if (output.shouldEmitNewRound) {
    // Enviar todas as unidades atualizadas
    const serializedUnits = allUnits.map((u) => ({
      id: u.id,
    ...
    io.to(lobbyId).emit("battle:new_round", {
      battleId,
      round: output.turnTransition.newRound,
