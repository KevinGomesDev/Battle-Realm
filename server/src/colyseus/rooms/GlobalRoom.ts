...
export class GlobalRoom extends Room<GlobalRoomState> {
  ...
  private registerMessageHandlers() {
    ...
    // Arena lobbies listing
    this.onMessage("arena:list_lobbies", async (client, _message) => {
      await this.handleListArenaLobbies(client);
    });
  }
  ...
  private async handleListArenaLobbies(client: Client) {
    try {
      const rooms = await matchMaker.query({ name: "arena" });
      ...
      client.send("arena:lobbies_list", lobbies);
    } catch (error) {
      client.send("arena:error", {
        message: "Erro ao listar lobbies de arena",
      });
    }
  }
  ...
  private async handleGetRanking(
    ...
    try {
      // Buscar ranking baseado no tipo (arena wins por padrão)
      const users = await prisma.user.findMany({
        orderBy:
          type === "wins" ? { arenaWins: "desc" } : { arenaWins: "desc" },
        take: limit,
        select: {
          id: true,
          username: true,
      ...
      client.send("ranking:data", {
        ...
        entries: users.map((u, idx) => ({
          ...
          userId: u.id,
          username: u.username,
          wins: u.arenaWins + u.matchWins,
          losses: u.arenaLosses + u.matchLosses,
  ...
  private async updateStats() {
    try {
      ...
      this.state.activeLobbies = arenaRooms.filter(
        (r) => r.metadata?.status === "WAITING"
      ).length;
      this.state.activeBattles =
        arenaRooms.filter((r) => r.metadata?.status === "BATTLING").length +
        matchRooms.length;
      ...
      for (const room of arenaRooms) {
        if (room.metadata?.status === "WAITING") {
          const lobby = new ArenaLobbyState();
