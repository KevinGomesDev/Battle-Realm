...
export async function validateSkillUsage(
  ...
  const skill = skillResult.ability;

  // Se é passiva, sempre pode ser usada (já está ativa)
  if (skill.activationType === "PASSIVE") {
...
export function listAllClasses() {
  return HERO_CLASSES.map((classDef) => ({
    ...
    skillCount: classDef.abilities.length,
    skills: classDef.abilities.map((skill: SkillDefinition) => ({
      id: skill.code,
      name: skill.name,
      ...
      range: skill.range,
      rangeSquares: skill.range ? DEFAULT_RANGE_VALUES[skill.range] ?? 0 : 0,
    })),
  }));
}
