// client/src/features/qte/hooks/useQTE.ts
...
export function useQTE({
  ...
  useEffect(() => {
    ...
    const handleQTEStart = (config: QTEConfig) => {
      console.log("[QTE] Recebido battle:qte:start", config);
    ...
    const handleQTEResolved = (result: QTEResult) => {
      ...
      // Manter QTE ativo brevemente para mostrar resultado
      setState((prev) => ({
        ...prev,
        result,
        hasResponded: true, // Marca como respondido para parar animação
      ...
      // Limpar QTE após delay para todos verem o resultado
      setTimeout(() => {
        setState((prev) => ({
          ...prev,
          activeQTE: null,
    ...
    colyseusService.on("battle:qte:start", handleQTEStart);
    colyseusService.on("battle:qte:resolved", handleQTEResolved);
    colyseusService.on("battle:qte:expired", handleQTEExpired);
    colyseusService.on("battle:qte:cascade", handleQTECascade);
    ...
    return () => {
      ...
      colyseusService.off("battle:qte:start", handleQTEStart);
      colyseusService.off("battle:qte:resolved", handleQTEResolved);
      colyseusService.off("battle:qte:expired", handleQTEExpired);
      colyseusService.off("battle:qte:cascade", handleQTECascade);
    };
  ...
  const isLocalResponder = Boolean(
    state.activeQTE &&
      localPlayerId &&
      // Verifica se o dono da unidade que responde é o jogador local
      state.activeQTE.responderOwnerId === localPlayerId
  );
  ...
  const respondToQTE = useCallback(
    (input: QTEInput, hitPosition: number) => {
      ...
      colyseusService.sendToBattle("qte:response", response);
    },
